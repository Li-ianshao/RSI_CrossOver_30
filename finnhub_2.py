...
headers = ['代碼', '收盤價', '配息日', '配息', '此次配息率', '殖利率', 'RSI', '當日漲跌幅', '一年最低價', '一年最高價', 'BBAND', 'volume_Delta', 'Surprise%']
...
for s in stocks:
	...
	if len(div_date) > 1 and div_date > date.today().strftime("%Y-%m-%d") and yields > 4.0:
		...
		df_res.loc[len(df_res.index)] = [s_, '$' + str(Close), div_date, '$' + str(div2), div_thisTime, str(yields) + '%', round(RSI,2), str(change)+'%', fiftyTwoWeeksLow, fiftyTwoWeeksHigh, str(BV)+'%', str(volume_Delta)+'%', Surprise] 
