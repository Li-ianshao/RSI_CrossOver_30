...
def stock_info():
    ...
    for s in ticker_list:
        ...
        if decreased >= threshold or increased > threshold:
            #print(s)
            if s in aristocrats:
                category = 'AR'
            elif s in SNP500:
            ...
            try:
                ...
                earningsQuarterlyGrowth = sys.float_info.max
                earningsGrowth = sys.float_info.max
            ...
            if (decreased >= threshold):
                t = {'Symbol': s, 'category': category, 'Industry':industry, 'Close': Close, 'Decrease': change, '3mDecrease':decreased, 'earningsQuarterlyGrowth':earningsQuarterlyGrowth, 'earningsGrowth':earningsGrowth, 'PERatio':trailingPE, 'EPS':trailingEps, 'Volume': Volume, 'AVG_Volume':averageVolume, 'Div':dividendRate, 'Yields': dividendYield, 'Div_Date':divDate, 'RSI': RSI, 'BBAND': BBAND, 'UBAND': UBAND, 'MBAND':MBAND, 'LBAND':LBAND, 'MACD': MACD}
                stock_data.append(t)
            else:
                t = {'Symbol': s, 'category': category, 'Industry':industry, 'Close': Close, 'Decrease': change, '3mDecrease':increased, 'earningsQuarterlyGrowth':earningsQuarterlyGrowth, 'earningsGrowth':earningsGrowth, 'PERatio':trailingPE, 'EPS':trailingEps, 'Volume': Volume, 'AVG_Volume':averageVolume, 'Div':dividendRate, 'Yields': dividendYield, 'Div_Date':divDate, 'RSI': RSI, 'BBAND': BBAND, 'UBAND': UBAND, 'MBAND':MBAND, 'LBAND':LBAND, 'MACD': MACD}
                inc_data.append(t)

    end = time.time()
    total = len(stock_data)
    total_inc = len(inc_data)
    stock_data = sorted(stock_data, key=itemgetter('3mDecrease'), reverse=True)
    inc_data = sorted(inc_data, key=itemgetter('3mDecrease'), reverse=True)
    print(f'更新網頁耗時：{round(end - start,2)} 秒')

    return render_template('stock_info.html', total=total, threshold=threshold, data1=stock_data, total_2=total_inc, data2=inc_data)
